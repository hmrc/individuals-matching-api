...
@Singleton
class MongoRepository @Inject()(mongo: MongoComponent, configuration: Configuration)(
  implicit ec: ExecutionContext)
    extends PlayMongoRepository[NinoMatch](
      ...
      replaceIndexes = true,
      indexes        = Seq(IndexModel(ascending("id"), IndexOptions().name("idIndex").unique(true).background(true)),
        IndexModel(ascending("createdAt"), IndexOptions().name("createdAtIndex").expireAfter(configuration.getOptional[Int]("mongo.ninoMatchTtlInSeconds").getOrElse(60 * 60 * 5), TimeUnit.SECONDS).background(true)))) {
  ...
  def read(id: UUID)(implicit ec: ExecutionContext): Future[Option[NinoMatch]] = {
